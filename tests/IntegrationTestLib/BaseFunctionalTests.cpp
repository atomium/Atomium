...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "atomiumd.exe";
#else
const std::string DAEMON_FILENAME = "atomiumd";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_atomiumd_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"atomiumd" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "atomiumd", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
